Deno.serve(async (req) => {
  ...
  try {
    ...
    const performanceResponse = await fetch(
      `${supabaseUrl}/rest/v1/ml_model_race_results${query}`,
      {
        method: 'GET',
        headers: {
    ...
    const summaryResponse = await fetch(
      `${supabaseUrl}/rest/v1/ml_model_race_summary`,
      {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${supabaseKey}`,
    ...
    const recentResponse = await fetch(
      `${supabaseUrl}/rest/v1/rpc/get_recent_ml_race_performance`,
      {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${supabaseKey}`,
    ...
    for (const model of models) {
      ...
      modelStats[model] = {
        ...
        average_confidence: modelData.length > 0 ? 
          (modelData.reduce((sum, p) => sum + p.predicted_probability, 0) / modelData.length) * 100 : 0
      };
    }

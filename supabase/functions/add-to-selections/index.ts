Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (race_id) {
      ...
      const raceEntryResponse = await fetch(`${supabaseUrl}/rest/v1/race_entries?race_id=eq.${encodeURIComponent(race_id)}&horse_name=ilike.${encodeURIComponent(horse_name)}&select=horse_id,race_id&limit=1`, {
        method: 'GET',
        headers: {
    ...
    if (!realHorseId || !realRaceId) {
      const fallbackResponse = await fetch(`${supabaseUrl}/rest/v1/race_entries?horse_name=ilike.${encodeURIComponent(horse_name)}&select=horse_id,race_id&limit=1`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${serviceRoleKey}`,
          'apikey': serviceRoleKey,
    ...
    const existingResponse = await fetch(`${supabaseUrl}/rest/v1/selections?user_id=eq.${userId}&horse_name=eq.${encodeURIComponent(horse_name)}&course_name=eq.${encodeURIComponent(course_name)}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${serviceRoleKey}`,
        'apikey': serviceRoleKey,

Deno.serve(async (req) => {
  ...
  try {
    ...
    const performanceResponse = await fetch(
      `${supabaseUrl}/rest/v1/ml_model_race_results?created_at=gte.${today}T00:00:00.000Z&created_at=lt.${today}T23:59:59.999Z&select=*&order=created_at.desc`,
      {
        method: 'GET',
        headers: {
    ...
    const racesResponse = await fetch(
      `${supabaseUrl}/rest/v1/race_results?date=eq.${today}&select=race_id`,
      {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${supabaseKey}`,
    ...
    const models = ['mlp', 'rf', 'xgboost', 'benter', 'ensemble'];
...
function getModelFullName(modelName: string): string {
  const names = {
  ...
  return names[modelName as keyof typeof names] || modelName;
}

// Helper function to get performance trend
function getPerformanceTrend(winRate: number): 'hot' | 'cold' | 'normal' {
...
async function getNextRunnerForModel(modelName: string, supabaseUrl: string, supabaseKey: string) {
  try {
    // Get today's date in UK timezone
    const today = new Date().toLocaleDateString('en-CA', { timeZone: 'Europe/London' });
    ...
    const nextRaceResponse = await fetch(
      `${supabaseUrl}/rest/v1/races?date=eq.${today}&order=off_time.asc&limit=1&select=race_id,off_time,course_name`,
      {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${supabaseKey}`,
    ...
    const entriesResponse = await fetch(
      `${supabaseUrl}/rest/v1/race_entries?race_id=eq.${nextRace.race_id}&select=horse_name,horse_id,trainer_name,jockey_name,current_odds,${modelName}_proba&${modelName}_proba=gt.0&order=${modelName}_proba.desc&limit=1`,
      {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${supabaseKey}`,

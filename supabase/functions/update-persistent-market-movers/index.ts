Deno.serve(async (req) => {
  ...
  try {
    ...
    const marketMovementResponse = await fetch(
      `${supabaseUrl}/rest/v1/horse_market_movement?select=*&order=last_updated.desc`,
      {
        method: 'GET',
        headers: {
    ...
    const raceIds = [...new Set(marketMovements.map((m: any) => m.race_id))];
    const raceDetailsResponse = await fetch(
      `${supabaseUrl}/rest/v1/races?race_id=in.(${raceIds.map(id => `"${id}"`).join(',')})&select=race_id,course_name,off_time,date`,
      {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${supabaseKey}`,
    ...
    const raceMap = new Map(raceDetails.map((r: any) => [r.race_id, r]));
    ...
    for (const movement of marketMovements) {
      ...
      const existingResponse = await fetch(
        `${supabaseUrl}/rest/v1/persistent_market_movers?horse_id=eq.${movement.horse_id}&race_id=eq.${movement.race_id}&select=id,is_active,odds_movement_pct`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${supabaseKey}`,
      ...
      if (existing.length === 0) {
        ...
        if (meetsCriteria) {
          const addResponse = await fetch(
            `${supabaseUrl}/rest/v1/persistent_market_movers`,
            {
              method: 'POST',
              headers: {
                'Authorization': `Bearer ${supabaseKey}`,
        ...
        if (meetsCriteria) {
          ...
          const updateResponse = await fetch(
            `${supabaseUrl}/rest/v1/persistent_market_movers?id=eq.${existingRecord.id}`,
            {
              method: 'PATCH',
              headers: {
                'Authorization': `Bearer ${supabaseKey}`,

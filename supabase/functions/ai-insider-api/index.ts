...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const dataResponse = await fetch(`${supabaseUrl}/functions/v1/ai-insider-data`, {
      method: 'POST',
      headers: {
    ...
    const transformedData = {
      ...
      data: {
        course_distance_specialists: dataResult.data.courseSpecialists.map((s, index)=>({
            id: index + 1,
            horse_id: s.horse_id,
            race_id: s.race_id,
        ...
        trainer_intents: dataResult.data.trainerIntents.map((t, index)=>({
            id: index + 1,
            horse_id: t.horse_id,
            race_id: t.race_id,
        ...
        market_movers: dataResult.data.marketAlerts.map((m, index)=>({
            id: index + 1,
            horse_id: m.horse_id,
            race_id: m.race_id,
      ...
      summary: {
        ...
        courses_covered: new Set([
          ...dataResult.data.courseSpecialists.map((s)=>s.course_name),
          ...dataResult.data.trainerIntents.map((t)=>t.course),
          ...dataResult.data.marketAlerts.map((m)=>m.course)
        ]).size,
        last_updated: new Date().toISOString()
      }
    };
    ...
    console.error('AI Insider API failed');

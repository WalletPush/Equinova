Deno.serve(async (req) => {
    ...
    try {
        ...
        console.log('Get user selections request:', { status, sortBy, sortOrder });
        ...
        const currentDate = now.toLocaleDateString('en-CA', { timeZone: 'Europe/London' }); // YYYY-MM-DD format

        console.log('Current UK date/time:', currentDate, currentTime);

        // Get all user selections
        const selectionsResponse = await fetch(
            `${supabaseUrl}/rest/v1/selections?user_id=eq.${userId}&order=${sortBy}.${sortOrder}`,
            {
                method: 'GET',
                headers: {
        ...
        allSelections.forEach((selection: any) => {
            try {
                ...
                const isFromYesterday = selectionDate === new Date(now.getTime() - 24 * 60 * 60 * 1000).toLocaleDateString('en-CA', { timeZone: 'Europe/London' });
                ...
                // If it's from today, compare times
                else if (isFromToday) {
                ...
                // If we can't determine the date, use time comparison as fallback
                else {
                    if (adjustedRaceTime > currentTime) {
                        upcomingSelections.push(selection);
                    } else {

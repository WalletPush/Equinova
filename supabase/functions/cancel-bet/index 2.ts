Deno.serve(async (req) => {
    ...
    try {
        ...
        console.log('Cancel bet request:', { bet_id });
        ...
        const getBetResponse = await fetch(
            `${supabaseUrl}/rest/v1/bets?id=eq.${bet_id}&user_id=eq.${userId}&select=*`,
            {
                method: 'GET',
                headers: {
        ...
        // Delete the bet
        const deleteBetResponse = await fetch(
            `${supabaseUrl}/rest/v1/bets?id=eq.${bet_id}&user_id=eq.${userId}`,
            {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${serviceRoleKey}`,
        ...
        const getBankrollResponse = await fetch(
        ...
        const updateBankrollResponse = await fetch(

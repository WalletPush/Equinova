Deno.serve(async (req) => {
  ...
  try {
    ...
    const startTimeInMinutes = 8 * 60; // 8:00 AM UK time - FIXED!
    ...
    console.log('Starting Racing API market monitoring (8:00am-9:00pm schedule) - ENHANCED VERSION...');
    ...
    // Step 2: Process ALL horses from Racing API (not just those in our database)
    // This FIXES the critical bug - we now capture any horse with market movement
    let processedCount = 0;
    let actuallyInsertedCount = 0;
    let ladbrokesOddsFoundCount = 0;
    let newHorsesFound = 0;
    
    console.log('ENHANCED MODE: Processing ALL horses from Racing API...');
    ...
    return new Response(JSON.stringify({
      success: true,
      message: 'Racing API market monitoring completed successfully (8:00am-9:00pm schedule) - ENHANCED VERSION CAPTURES ALL HORSES',
